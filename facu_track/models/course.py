...
class Course(models.Model):
    ...
    available_time = fields.Datetime(string="Available Time",readonly=True)
    ...
    def access_location(self):
        ...
        if datetime.now() > self.available_time:
            raise ValidationError("Sorry The Attendance Time Has Finished")
    ...
    # def check_availability_of_attend(self):
    #     available_time = datetime.now() + timedelta(hours=2,minutes=self.start)
...
class StartAttend(models.TransientModel):
    _name = 'facu_track.start'

    start_attend = fields.Float(string="Start Attendance")
    course_id = fields.Many2one('facu_track.course')

    def start_attendance(self):
        if self.course_id:
            self.course_id.write({
                'available_time':datetime.now()+timedelta(minutes=self.start_attend)
            })
